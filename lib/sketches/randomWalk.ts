...
import type { SketchSettings, SketchAnimated } from "../types";
...
export const sketch = (): SketchAnimated => {
  ...
  function hexToHsla(hex: string, a = 1) {
  ...
  const draw = (context: CanvasRenderingContext2D, wanderer: Wanderer) => {
    context.beginPath();
  ...
  class Wanderer {
    ...
    constructor(startX = canvasWidth / 2, startY = canvasHeight / 2) {
      this.x = startX;
      this.y = startY;
      this.prevX = this.x;
      this.prevY = this.y;
